...
namespace Springie.autohost
{
    ...
    public class VotePlanet: AbstractPoll, IVotable
    {
        ...
        public VotePlanet(TasClient tas, Spring spring, AutoHost ah): base(tas, spring, ah)
        {
            ...
            ISpringieServer pw = ah.PlanetWars.server;
            IFaction fact = pw.GetOffensiveFaction(ah.PlanetWars.account);
            faction = fact.Name;
            ...
            if (b != null)
            {
                foreach (UserBattleStatus us in b.Users)
                {
                    if (us.Name != tas.UserName && !us.IsSpectator)
                    {
                        IPlayer ui = pw.GetPlayerInfo(ah.PlanetWars.account, us.Name);
                        if (ui != null && ui.FactionName == faction)
                        {
                            users.Add(us.Name);
                            votes.Add(0);
                            initialUserCount++;
        ...
        bool IVotable.Init(TasSayEventArgs e, string[] words)
        {
            ...
            string[] vals;
            int[] indexes;
            if (ah.PlanetWars.FilterPlanets(words, out vals, out indexes) > 0)
            {
        ...
        public bool Vote(TasSayEventArgs e, string[] words)
        {
            ...
            if (e.UserName != "")
            {
                ...
                IPlayer info = pw.GetPlayerInfo(ah.PlanetWars.account, e.UserName);
            ...
            if (CheckEnd(out winVote))
            {
                if (winVote == 1)
                {
                    ...
                    IPlanet sel = pw.GetAttackOptions(ah.PlanetWars.account).Where(p => p.Name == planet).Single();
                    var mapi = Program.main.UnitSyncWrapper.MapList[sel.MapName];
                    tas.ChangeMap(mapi.Name, mapi.Checksum);
                }
