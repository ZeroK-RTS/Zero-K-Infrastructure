...
namespace LobbyClient
{
	public class TasClient
	{
		...
		public event EventHandler<CancelEventArgs<TasSayEventArgs>> PreviewSaidPrivate = delegate { };
		...
		void DispatchServerCommandOnGuiThread(string command, string[] args)
		{
			...
			try
			{
				...
				switch (command)
				{
					...
					case "MOTD": // server motd
						if (args.Length > 0) Said(this, new TasSayEventArgs(TasSayEventArgs.Origins.Server, TasSayEventArgs.Places.Motd, "", "", Utils.Glue(args, 0), false));
						break;

					case "SERVERMSG": // server message
						Said(this, new TasSayEventArgs(TasSayEventArgs.Origins.Server, TasSayEventArgs.Places.Server, "", "", Utils.Glue(args, 0), false));
						break;
					...
					{
						var sayArgs = new TasSayEventArgs(TasSayEventArgs.Origins.Player, TasSayEventArgs.Places.Channel, args[0], args[1], Utils.Glue(args, 2), false);
						Said(this, sayArgs);
						break;
					}
					case "RING":
					...
					{
						var sayArgs = new TasSayEventArgs(TasSayEventArgs.Origins.Player, TasSayEventArgs.Places.Channel, args[0], args[1], Utils.Glue(args, 2), true);
						Said(this, sayArgs);
						break;
					}
					...
					{
						var sayArgs = new TasSayEventArgs(TasSayEventArgs.Origins.Player, TasSayEventArgs.Places.Normal, args[0], username, Utils.Glue(args, 1), false);
						var user = ExistingUsers[sayArgs.Channel];
						var me = ExistingUsers[UserName];
						var previewSaidEventArgs = new CancelEventArgs<TasSayEventArgs>(sayArgs);
						PreviewSaidPrivate(this, previewSaidEventArgs);
						if (!previewSaidEventArgs.Cancel) Said(this, sayArgs);
						// channel = char partner Name
					}
						break;

					case "SAIDPRIVATE": // someone said something to me
					{
						var sayArgs = new TasSayEventArgs(TasSayEventArgs.Origins.Player, TasSayEventArgs.Places.Normal, args[0], args[0], Utils.Glue(args, 1), false);
						var user = ExistingUsers[sayArgs.UserName];
						var previewSaidEventArgs = new CancelEventArgs<TasSayEventArgs>(sayArgs);
						PreviewSaidPrivate(this, previewSaidEventArgs);
						if (!previewSaidEventArgs.Cancel) Said(this, sayArgs);
						break;
					...
					case "CLIENTSTATUS": // client's status changed
					{
						...
						if (MyBattle != null && MyBattle.Founder == u.Name)
						{
							...
							if (!u.IsInGame && old.IsInGame == true) MyBattleHostExited(this, new TasEventArgs());
						}
					...
					case "LEFTBATTLE": // user left the battle
					{
						...
						var user = args[1];
						var battle = ExistingBattles[battleID];
					...
					case "AQUIREUSERID":
						con.SendCommand("USERID", NetworkInterface.GetAllNetworkInterfaces().First().GetPhysicalAddress().GetHashCode().ToString("x").ToUpper());
						break;
					case "ADDSTARTRECT":
					{
			...
			{
				var newEx = new ApplicationException(string.Format("Error while processing command {0} {1}: {2}", command, Utils.Glue(args), e.Message), e);
