...
namespace PlasmaShared
{
    public class MetaDataCache
    {
        ...
        Dictionary<string, List<MapRequestCallBacks>> currentMapRequests = new Dictionary<string, List<MapRequestCallBacks>>();
        Dictionary<string, List<ModRequestCallBacks>> currentModRequests = new Dictionary<string, List<ModRequestCallBacks>>();
        // these metadata requests are ongoing

        object mapRequestsLock = new object();
        object modRequestsLock = new object();

        // ok its ugly hardcoded, but service stub itself has it hardcoed :)

        readonly string resourceFolder;
        SpringScanner scanner;
        WebClient webClientForMap = new WebClient() { Proxy = null };
        WebClient webClientForMod = new WebClient() { Proxy = null };
        ...
        public Mod GetModMetadata(byte[] data)
        {
            ...
            var hash = scanner.GetSpringHash(ret.Name);
            if (hash != 0) ret.Checksum = hash;
            foreach (Option option in ret.Options)
            {
                if (option.Type == OptionType.Number) option.Default = option.Default.Replace(",", ".");
            }
            return ret;
        }
        ...
        class MapRequestCallBacks
        {
            ...
            public MapCallback SuccessCallback;
        ...
        {
            public Action<Exception> ErrorCallback;
            public Action<Mod> SuccessCallback;
