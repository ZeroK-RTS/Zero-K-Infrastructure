...
namespace CMissionLib.UnitSyncLib
{
    public partial class UnitSync: IDisposable
    {
        ...
        IEnumerable<Ai> GetAis()
        {
            ...
            var ais = new List<Ai>();
            for (var i = 0; i < NativeMethods.GetSkirmishAICount(); i++) ais.Add(new Ai { Info = GetAiInfo(i).ToArray(), Options = GetAiOptions(i).ToArray() });
            return ais;
        }
        ...
        {
            FIBITMAP dib;
            using (var memoryStream = new MemoryStream(data)) dib = FreeImage.LoadFromStream(memoryStream);
            try
            {
                using (var bitmap = FreeImage.GetBitmap(dib)) return bitmap.ToBitmapSource();
            }
            finally
            {
