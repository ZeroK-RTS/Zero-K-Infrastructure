	...
	public partial class DownloadBar: UserControl, INotifyBar
	{
		...
		public void UpdateInfo() {
			if (Download == null) // Probably will never happen.
			{
				...
				var dl = (long)(Download.TotalProgress/100.0*Download.Length);
				var speed = Download.AverageSpeed;
				if (Download.IsComplete == false) label.Text = Download.Name + " Failed";
					...
					label.Text = string.Format("{0} - {1} remaining - {2:F1}B of {3:F1}B - {4:F1}B/s",
					                           ...
					                           Utils.PrintByteLength(Download.Length),
					                           Utils.PrintByteLength(speed));
				}
		...
		public void AddedToContainer(NotifyBarContainer container) {
			container.btnDetail.Image = Resources.XferDown;
		}
		...
		private void minimapImageLoadComplete(object sender, DownloadDataCompletedEventArgs e) {
