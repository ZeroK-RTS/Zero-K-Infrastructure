    ...
    public partial class ChatControl: UserControl
    {
        ...
        void completeNick()
        {
            ...
            if (!nickCompleteMode)
            {
                ...
                var ncMatches = playerBox.GetUserNames().Where(x => x.ToLower().Contains(ncWordToComplete.ToLower())).ToList();
            ...
            if (nickCompleteMode)
            {
                ...
                //remake chatbox text
                sendBox.Text = ncFirstChunk + nick + ncSecondChunk;
                sendBox.SelectionStart = ncFirstChunk.Length + nick.Length;
            }
        }
        ...
        private void client_HourChime(object sender, EventArgs e)
        {
        ...
        void sendBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\t')
            {
                completeNick();
        ...
        void sendBox_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            //Prevent cutting line in half when sending
            if (e.KeyCode == Keys.Return) sendBox.SelectionStart = sendBox.Text.Length;
