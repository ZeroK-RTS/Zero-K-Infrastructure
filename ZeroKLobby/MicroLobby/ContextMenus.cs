...
namespace SpringDownloader.MicroLobby
{
    ...
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability", "CA2000:Dispose objects before losing scope"), System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Globalization", "CA1303:Do not pass literals as localized parameters", MessageId = "System.Windows.Forms.Menu+MenuItemCollection.Add(System.String)")]
                    ...
                    item.Click += (s, e) =>
                        {
                            ...
                            var colorDialog = new ColorDialog
                                              {
                                                  Color = Color.FromArgb(botColor[0], botColor[1], botColor[2]),
                                                  Site = Program.FormMain.ChatTab.Site
                                              };
        ...
        public static ContextMenu GetChannelContextMenu(ChatControl chatControl)
        {
            ...
            try
            {
                ...
                {
                    var autoJoinItem = new MenuItem("Automatically Join Channel")
                        ...
                        if (user.Name != Program.TasClient.UserName)
                        {
                            ...
                            if (battleStatus.TeamNumber == myStatus.TeamNumber)
                            {
                                var unShareControl = new MenuItem("Unshare Command")
                                                     {
                                                         Enabled =
                                                             !battleStatus.IsSpectator && !myStatus.IsSpectator &&
                                                             (myStatus.Name != battleStatus.Name)
                                                     };
                                ...
                                var shareControl = new MenuItem("Share Command")
                                                   {
                                                       Enabled =
                                                           !battleStatus.IsSpectator &&
                                                           (battleStatus.TeamNumber != myStatus.TeamNumber || myStatus.IsSpectator)
                                                   };
            ...
            if (Program.ModStore.Ais != null)
            {
                foreach (var bot in Program.ModStore.Ais)
                {
                    ...
                    item.Click +=
                        (s, e) => {
                                      var botNumber =
                                          Enumerable.Range(1, int.MaxValue).First(
                                              i => !Program.TasClient.MyBattle.Bots.Any(bt => bt.Name == "Bot_" + i));
                                      Program.TasClient.AddBot("Bot_" + botNumber,
                                                               Program.TasClient.MyBattleStatus.Clone(),
                                                               (int)(MyCol)Color.White,
                                                               b.ShortName);
                        };
