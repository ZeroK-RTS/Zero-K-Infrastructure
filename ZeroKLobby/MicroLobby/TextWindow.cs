...
namespace ZeroKLobby.MicroLobby
{
    public partial class TextWindow : ZklBaseControl
    {
        ...
        private readonly List<DisplayLine> displayLines;
        private readonly List<TextLine> textLines;
        private readonly Regex wwwRegex = new Regex(WwwMatch);
        ...
        public void ScrollWindowPage(bool scrollUp) //seems unused
        {
            try
            {
                ...
                if (scrollUp)
                {
                    if (vScrollBar.Value > vScrollBar.LargeChange) vScrollBar.Value = vScrollBar.Value - vScrollBar.LargeChange;
                    else vScrollBar.Value = 1; //reach top
                    Invalidate();
                }
                else
                {
                    if (vScrollBar.Value < TotalDisplayLines - vScrollBar.LargeChange) vScrollBar.Value = vScrollBar.Value + vScrollBar.LargeChange;
                    else vScrollBar.Value = TotalDisplayLines; //reach bottom
                    Invalidate();
                }
            }
            catch (Exception e)
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            try
            {
                if (startHighLine > -1 && curHighLine > -1)
                {
                    ...
                    {
                        var sw = startHighLine; //shift up/down role
        ...
        public void AppendText(string newLine, int color = TextColor.text)
        {
            try
            {
                ...
                var isSpam = DateTime.Now.Ticks - previousAppendText <= 2000000; //2,000,000 ticks is 200milisecond.
                ...
                var needSpace = totalLines >= MaxTextLines;
                var isExpanding = MaxTextLines < defaultMaxLines;
                var isFull = MaxTextLines >= HardMaximumLines;
                if (needSpace && !isExpanding && (isFull || !isSpam))
                {
                    ...
                    {
                        if (displayLines[wrapCount + lineCount].Previous) wrapCount++;
                        else lineCount++;
                    }
                    var vScrollBarOffset = wrapCount + lineCount - 1;
                    vScrollBarOffset = Math.Min(vScrollBarOffset, vScrollBar.Value - 1);

                    //shift existing texts upward:
                    var x = 0;
                    for (var i = toRemove; i < totalLines; i++)
                    {
                ...
                else if (needSpace && !isFull && (isExpanding || isSpam)) AddNewTextLines(); //create new space for new text (new row)
                ...
                textLines[totalLines].Width = MS(g, TextColor.StripCodes(newLine), Font);

                g.Dispose();

                textLines[totalLines].TextColor = foreColor;

                var addedLines = FormatLines(totalLines, totalLines, TotalDisplayLines); //split text into multi-line, and put into "displayLines[]"
                addedLines -= TotalDisplayLines;

                textLines[totalLines].TotalLines = addedLines;

                for (var i = TotalDisplayLines + 1; i < TotalDisplayLines + addedLines; i++) displayLines[i].TextLine = totalLines; //identify to current "textLines[]" index
...
*/
        ...
        private int DT(Graphics g, StringBuilder buildString, Font font, int startX, int startY, int foreColor, int curBackColor)
        {
            return DT(g, buildString.ToString(), font, startX, startY, foreColor, curBackColor);
        }
            ...
            for (var currentLine = endLine; currentLine <= startLine; currentLine++)
            {
                ...
                if (textLines[currentLine].Width <= displayWidth)
                {
                    try
                    {
                        ...
                        line++;
                        if (line >= MaxDisplayLines) AddNewDisplayLines();
                    }
                    catch (Exception e)
                    {
        ...
        private int MS(Graphics g, StringBuilder buildString, Font font) => MS(g, buildString.ToString(), font);
        ...
        private string RedefineColorCodes(string line)
        {
            ...
            var foreColorNest = new int[32]; //try to record/remember up to 32 color nest
            var backColorNest = new int[32];
            var foreColorInd = 0;
            var backColorInd = 0;
            ...
            while (m.Success)
            {
                ...
                else if (Regex.Match(m.Value, parseColorChar).Success)
                {
                    ...
                    var prevForeColor = foreColorNest[foreColorInd];
                    var backForeColor = backColorNest[backColorInd];
                    ...
                    for (var i = 0; i < line.Length; i++)
                    {
                        ...
                        if ((int)lookWidth >= x && foundSpace)
                        {
                            if (displayLines[lineNumber].Previous && lineNumber > 0 && space == 0)
                            {
                                ...
                                var prevwidth = MS(g, prevline, Font);
                                g.Dispose();
        ...
        private void UpdateScrollBar(int newValue, int offsetValue)
        {
            ...
            {
                var nearBottom = vScrollBar.Value + 5 >= TotalDisplayLines;
                var isBottom = vScrollBar.Value >= 1 + vScrollBar.Maximum - vScrollBar.LargeChange; //exactly at UI's scrollbar bottom
        ...
        private delegate void ScrollValueDelegate(int value, int offset);
