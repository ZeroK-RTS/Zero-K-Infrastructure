    ...
    public partial class MainWindow: Window
    {
        ...
        public void PopupSelf()
        {
            if (Dispatcher.CheckAccess())
            {
                ...
                if (wasminimized) WindowState = WindowState.Maximized;
                Show();
                Activate();
                Focus();
        ...
        void UpdateDownloads()
        {
            try
            {
                if (Program.Downloader != null && !Program.CloseOnNext)
                {
                    ...
                    foreach (
                        var pane in
                            new List<INotifyBar>(Program.NotifySection.Bars).OfType<DownloadBar>().Where(
                                x => x.Download.IsAborted || x.Download.IsComplete == true)) Program.NotifySection.RemoveBar(pane);
