...
{
	/// <summary>
	/// Strucutre encapsulating MD5 hash - can be easilly converted to string or byte[]
	/// </summary>
	[Serializable]
	public struct Hash : ICloneable
	{
		...
		{
			get { return BytesToString(data); }
			set { data = StringToBytes(value); }
		}
		...
		{
			return (Hash)new MD5CryptoServiceProvider().ComputeHash(Encoding.Unicode.GetBytes(s));
		}
		...
		static int CharToHexByte(char x)
		{
			if (x >= '0' && x <= '9') {
				...
				if (x >= 'a' && x <= 'f') {
					...
					throw new ArgumentException("character is not convertible to hex byte");
				}
			}
		}
		...
		static byte[] StringToBytes(string s)
		{
			...
			while (i < destLen) {
				res[i++] = (byte)((CharToHexByte(s[si++]) << 4) + CharToHexByte(s[si++]));
			}
			return res;
		}
