        ...
        private static void UpdateRatings(bool init = false)
        {
            ...
            try {
                foreach (Category cat in maps.Keys)
                {
                    ...
                    using (var db = new ZkDataContext())
                    {
                        var newRanking = maps[cat].Values.Where(x => db.Resources.First(y => y.ResourceID == x.id).MapSupportLevel != MapSupportLevel.None).Select(x => x.days[0]).OrderByDescending(x => x.r).ToList();
