        ...
        private static void UpdateRatings(bool init = false)
        {
            ...
            try
            {
                using (var db = new ZkDataContext())
                {
                    ...
                    outcomes = db.MapPollOutcomes.Where(x => x.MapPollID > lastPollId && x.MapPollID < endPollId && !x.MapPollOptions.Any(o => o.Resource.MapSupportLevel < MapSupportLevel.Supported)).Include(x => x.MapPollOptions).OrderBy(x => x.MapPollID).AsNoTracking().ToList();
                }
