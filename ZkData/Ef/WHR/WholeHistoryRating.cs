                ...
                else
                {
                    ...
                    if (RatingSystems.Initialized)
                    {
                        ...
                        using (var db = new ZkDataContext()) {
                            db.SpringBattlePlayers.Where(p => p.SpringBattleID == battle.SpringBattleID && !p.IsSpectator).ForEach(p => p.EloChange = playerRatings[p.AccountID].RealElo - playerOldRatings[p.AccountID].RealElo);
                            db.SaveChanges();
                        }
