                    ...
                    try
                    {
                        lock (updateLockInternal)
                        {
                            ...
                            using (var db = new ZkDataContext())
                            {
                                ...
                                db.SpringBattlePlayers.Where(p => p.SpringBattleID == latestBattle.SpringBattleID && !p.IsSpectator).AsNoTracking().ForEach(x => playerOldRatings[RatingSystems.GetRatingId(x.AccountID)] = playerRatings[RatingSystems.GetRatingId(x.AccountID)]);
                                db.SaveChanges();
                            }
