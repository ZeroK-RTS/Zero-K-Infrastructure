	...
	public class MyController: Controller
	{
		...
		void GetUnlockLists(ZkDataContext db, out List<Unlock> unlocks, out List<Unlock> future)
		{
			...
			var temp =
				db.Unlocks.Where(
					x =>
					x.NeededLevel <= Global.Account.Level && ((x.XpCost <= Global.Account.AvailableXP && x.IsKudosOnly != true) || x.KudosCost <= Global.Account.KudosGained ) && !maxedUnlockList.Contains(x.UnlockID) &&
