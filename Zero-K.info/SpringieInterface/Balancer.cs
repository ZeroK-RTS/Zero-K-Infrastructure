	...
	public class Balancer
	{
		public static BalanceTeamsResult BalanceTeams(BattleContext context, bool isGameStart, int? allyCount, bool? clanWise) {
			...
			if (!isGameStart) {
				...
				if (playerCount%2 == 1 && playerCount < 5) res.CanStart = false;
		...
		private static void VerifySpecCheaters(BattleContext context, BalanceTeamsResult res) {
			try {
				...
				using (var db = new ZkDataContext()) {
					...
					var ipByLobbyID = db.Accounts.Where(x => ids.Contains(x.LobbyID)).ToDictionary(x => x.LobbyID, x => x.AccountIPS.OrderByDescending(y => y.LastLogin).First().IP); // lobbyid -> ip mapping
