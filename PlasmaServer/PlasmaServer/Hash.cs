...
namespace PlasmaServer
{
    ...
    public struct Hash : ICloneable
    {
        ...
        {
            foreach (DirectoryInfo di in folder.GetDirectories()) HashFolderRecursive(di, Utils.MakePath(path, di.Name), entries);
            foreach (FileInfo fi in folder.GetFiles()) entries.Add(Utils.MakePath(path, fi.Name), fi);
        }
