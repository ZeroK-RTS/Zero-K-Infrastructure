            ...
            public bool CanBeAdded(PlayerEntry other, List<PlayerEntry> allPlayers, bool ignoreSizeLimit)
            {
             ...
             /*   if (hasParty)           
                ...
                /*    if (!hasParty)
                    ...
                    if ((((GetPartyMinElo(other.Party, allPlayers) - MinElo )*(1+DynamicConfig.Instance.MmWidthReductionForParties))> width) || ((MaxElo - GetPartyMaxElo(other.Party, allPlayers)*(1+DynamicConfig.Instance.MmWidthReductionForParties)) > width))    //0% dynamic config will not boost and elo for party. 100% elo config will make parties fight with 2x stronger enemy
                    {
